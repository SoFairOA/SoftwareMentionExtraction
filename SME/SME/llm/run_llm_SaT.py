...
from langchain_aws import ChatBedrock, ChatPromptAdapter
    ...
    async with semaphore:
        ...
        while retries < max_retries:
            try:
                ...
                #-----USE ChatPromptAdapter TO STRUCTURE SYSTEM AND USER MESSAGES MORE EFFECTIVELY FOR THE MODEL-----#
                adapter = ChatPromptAdapter(system_message=system_message, human_message=user_message)
                
                #-----SEND THE REQUEST TO THE LLM USING THE ADAPTER TO STRUCTURE THE MESSAGES-----#
                response = await llm.abatch(inputs=[[adapter]])
